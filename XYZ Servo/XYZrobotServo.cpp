...
extern unsigned char XBEETxBuffer[];

// XYZrobotServo::XYZrobotServo(Stream & stream, uint8_t id)
XYZrobotServo::XYZrobotServo(uint8_t id)
{
  // this->stream = &stream;
  this->id = id;
  ...
  uint8_t * data2, uint8_t data2Size)
{
  ...
  uint8_t byteCount = 0x00; // stream->readBytes(header, sizeof(header)); $$$$
  ...
  if (data1Size)
  {
    byteCount = 0x00; // stream->readBytes(data1, data1Size);
  ...
  if (data2Size)
  {
    byteCount = 0x00; // stream->readBytes(data2, data2Size);
